...
bool aes_256_gcm_decrypt(uint64_t ciphertextSize, uint8_t *ciphertext,
                         ...
                         uint8_t tag[AES_BLOCK_SIZE], uint8_t *plaintext) {
  aes_init();
  return 0 == sgxsd_aes_gcm_run(false, key, ciphertext, ciphertextSize, plaintext, iv, nullptr, 0, tag);  
}

sgx_status_t sgxsd_aes_ctr_run(bool encrypt, const uint8_t p_key[SGXSD_AES_GCM_KEY_SIZE],
                               const void *p_src, uint32_t src_len, void *p_dst,
                               const uint8_t p_iv[SGXSD_AES_GCM_KEY_SIZE]) {
  if (p_key == NULL ||
((p_src == NULL || p_dst == NULL) && src_len != 0) ||
...
bool aes_256_ctr_decrypt(uint64_t ciphertextSize, uint8_t *ciphertext,
                         ...
                         uint8_t *plaintext) {
  aes_init();
  return 0 == sgxsd_aes_ctr_run(false, key, ciphertext, ciphertextSize, plaintext, iv);
}

RandGen::RandGen() {
    uint64_t random_seed = rd();
    const br_block_ctr_class * aes_vtable = &br_aes_x86ni_ctr_vtable; // in theory we should use br_aes_x86ni_ctr_get_vtable() here, but it seems bearssl does not recognize the cpuid and always return NUll
    // if (aes_vtable == NULL) {
    //   printf("vtable fail\n");
    //   throw 1;
    // }
    ...
    // printf("digest length = %ld bytes\n", (br_sha256_vtable.desc >> 8) & 0x7FUL);
}
